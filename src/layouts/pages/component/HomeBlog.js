    ...
    <div className="mt-4">
      {blogData.map((blog, index) => (
        <div key={index}>
          ...
          {activeIndex === index && <div className="leftSpace"> <div className="blogContent" dangerouslySetInnerHTML={{ __html: blog.content }}/><Link className="btnLink" to={blog.url}>Read More  <i class="bi bi-arrow-right"></i></Link></div>}
          </div>
        </div>
      ))}
    </div>
